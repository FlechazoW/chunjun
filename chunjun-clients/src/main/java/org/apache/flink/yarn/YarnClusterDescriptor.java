...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
    ...
    private ClusterClientProvider<ApplicationId> deployInternal(
        ...
        if (clusterSpecification.isCreateProgramDelay()) {
            ...
            for (File file : shipFiles) {
                tempFile.add(file.toURI().toURL());
            }
            clusterSpecification.setClasspaths(tempFile);
        ...
        if (jobGraph != null) {
            userJarFiles.addAll(
                    jobGraph.getUserJars().stream()
                            .map(org.apache.flink.core.fs.Path::toUri)
                            .map(Path::new)
    ...
    public String getClusterDescription() {
        ...
        try {
            ...
            ps.append("NodeManagers in the ClusterClient ").append(String.valueOf(metrics.getNumNodeManagers()));
            List<NodeReport> nodes = yarnClient.getNodeReports(NodeState.RUNNING);
